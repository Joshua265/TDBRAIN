...
def autopreprocess_standard(varargsin, subject=None, startsubj=0):
    ...
    if len(csv.files) < 1:
        raise ValueError(
            "no csv files found in this specified path, please check your sourcepath "
            + sourcepath
        )
    ...
    subs = [
        s
        for s in os.listdir(sourcepath)
        if os.path.isdir(os.path.join(sourcepath, s))
        if not any([e in s for e in ["preprocessed", "results", "DS"]])
    ]
    subs = np.sort(subs)
    ...
    for s in subarray:
        ...
        sessions = [
            session
            for session in os.listdir(os.path.join(sourcepath, subs[s]))
        ...
        for sess in range(len(sessions)):
            ...
            allconds = np.array(
                [
                    conds
                    for conds in os.listdir(
                        os.path.join(sourcepath, subs[s], sessions[sess] + "/eeg/")
                    )
                    if not any([e in conds for e in ["preprocessed", "results", "DS"]])
                ]
            )
            if reqconds == "all":
                conditions = allconds
            ...
            for c in range(len(conditions)):
                ...
                if len(conditions) > 0:
                    inname = os.path.join(
                        sourcepath, subs[s], sessions[sess] + "/eeg/", conditions[c]
                    )
                    # try:
                    tmpdat = ds(inname)
                    tmpdat.loaddata()
                    ...
                    sesspath = os.path.join(
                        preprocpath, subs[s], sessions[sess] + "/eeg/"
                    )
